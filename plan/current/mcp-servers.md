...
**Source:** Community MCP server wrapping [obsidian-local-rest-api](https://github.com/coddingtonbear/obsidian-local-rest-api)
**Credentials:** `OBSIDIAN_API_KEY` (generated by the Local REST API plugin in Obsidian settings)
**Prerequisite:** Obsidian must be running with the Local REST API plugin enabled

### What it does
Full read/write access to an Obsidian vault via the Local REST API plugin: create, read, update, and delete any note; full-text search across the vault; Dataview DQL queries; periodic note access (daily/weekly); tag listing.

### How the swarm uses it
**Swarm → Obsidian (write):** RESEARCH tasks deposit structured findings as vault notes. VIABILITY_REVIEW writes a score + rationale note per branch. On terminal state, the run summary is written as a dated note linking all outputs.

**Obsidian → Swarm (read):** DECOMPOSE searches the vault for prior domain knowledge before planning. RESEARCH checks for existing notes on a topic before going to web sources — avoiding redundant research across runs.

**Human annotation loop:** The operator reads and annotates swarm-written notes in Obsidian between runs. On the next DECOMPOSE, those annotations are read as context — closing the Human on the Loop feedback cycle without requiring the operator to interact with swarm.db.

**Primary task types:** `DECOMPOSE`, `RESEARCH`, `VIABILITY_REVIEW`
**Example invocations:** writing a research summary to `ALS/runs/2026-04-22/niche-research.md`; searching the vault for prior findings on "XBRL"; reading a human annotation left on a previous viability report.
...
| Obsidian MCP | `OBSIDIAN_API_KEY` | Yes (if Obsidian in use) | Obsidian → Settings → Local REST API plugin → API Key |
